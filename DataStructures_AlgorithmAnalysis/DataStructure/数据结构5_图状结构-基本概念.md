- [数据结构：图结构 - 基本概念](#数据结构图结构---基本概念)
  - [1. 基本概念](#1-基本概念)
    - [1.1. 图的二元组定义](#11-图的二元组定义)
    - [1.2. 有向图和无向图](#12-有向图和无向图)
    - [1.3. 度](#13-度)
    - [1.4. 稀疏图和稠密图](#14-稀疏图和稠密图)
    - [1.5. 图的分类](#15-图的分类)
    - [1.6. 子图和补图](#16-子图和补图)
    - [1.7. 图的同构](#17-图的同构)
    - [1.8. 通路与回路](#18-通路与回路)
  ...
  - [2. 存储结构](#2-存储结构)
    - [2.1. 邻接矩阵](#21-邻接矩阵)
    - [2.2. 邻接表](#22-邻接表)
    - [2.3. 十字链表（有向图）](#23-十字链表有向图)
    - [2.4. 邻接多重表（无向图）](#24-邻接多重表无向图)
  - [3. 无权图的应用](#3-无权图的应用)
    - [3.1. 魔棒抠图 Flood Fill](#31-魔棒抠图-flood-fill)
    - [3.2. 扫雷](#32-扫雷)
    - [3.3. 走迷宫](#33-走迷宫)
    - [3.4. 生成迷宫](#34-生成迷宫)
    - [3.5. 地图填色](#35-地图填色)
  - [4. 有权图的应用](#4-有权图的应用)
    - [4.1. AOV 和 AOE](#41-aov-和-aoe)
    - [4.2. 最小生成树](#42-最小生成树)
    - [4.3. 最短路径](#43-最短路径)
...
- 稀疏图 (Sparse Graph)：**当图中边的个数远小于该图所能拥有的最大边数时，称为稀疏图**。
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/4/17/905efc361deb665099ac9927efc411e6.jpg)

- 稠密图 (Dense Graph)：**当图中边的个数接近于该图所能拥有的最大边数时，称为稠密图**。当图中边的个数等于该图所能拥有的最大边数时，称为**完全图**。

  ![image](http://img.cdn.firejq.com/jpg/2018/4/17/6dbfb4aca86b8beecbb19bfad99c8785.jpg)
...
- 正则图：设 G 为 n 阶无向简单图，若对于任意 v ∈ V（G），均有 d(v)=k, 则称 G 为 k - 正则图，即 k - 正则图中每个结点的度数都等于 k。
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/fcea8216d1e0a11f6a65b636858108a9.jpg)

- 环图：如果图 G =（V，E）的结点集 V={v1, v2, ..., vn}（n >= 3），边集 E={(v1, v2), (v2, v3), , ( vn-1, vn), (vn, v1)}，则称 G 为环图，记为 Cn。环图都是 2 - 正则图。

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/12d046cec77472d8e4c19928090f7012.jpg)

- 轮图：当给环图 Cn-1（n >= 4）添加一个结点，并把这个结点和 Cn-1 里的每个结点逐个连接后得到的图成为轮图，记作 Wn。

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/d608c08b4dd74cb74920e976ba8604e3.jpg)

- 方体图：如果图 G =（V，E）有 2^n 个结点，每个结点表示一个长度为 n 的位串，任何两个相邻的结点表示的位串只有一位不同，则称 G 称为 n 方体图，记作 Qn。

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/9ffdcf42db33caf1d5def7e8cd822d8e.jpg)

- 二分图：如果图 G =（V，E）的结点集 V 能划分为两个子集：V1 和 V2，使每条边有一个端点在 V1 中，另一个端点在 V2 中，则称该图为二分图（或二部图）。

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/1b70f42ead3e89de6c7516fd3db95d27.jpg)

  - 完全二分图：二分图 G =（V，E）的结点集 V 能划分为两个子集：V1 和 V2，若 V1 中的每个结点和 V2 中的每个结点均有边相连，则称 G 为完全二分图。若|V1|=m，|V2|=n, 则可记为 Km,n。下图所示的是 K2,3 和 K3,3：

    ![image](http://img.cdn.firejq.com/jpg/2018/2/24/1b7ad9b984c8c7c1c12aaea380bc397f.jpg)

- 带权图 / 网：每个结点或每条边都带有数值的图称为带权图 / 网。

  下图左边为边带权图，右边为结点带权图：

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/d8cd0279d932acf739119fc8ba3ea308.jpg)
...
- 子图
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/37bca8a198dd7dc635105beff87d83c7.jpg)
  ```
...
![image](http://img.cdn.firejq.com/jpg/2018/2/24/c9d1532e2f14d8eefc3616e349b3b669.jpg)
...
![image](http://img.cdn.firejq.com/jpg/2018/2/24/0724111edac794ff3a3c7fb4579d9ac8.jpg)
...
- 例：
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/4/17/35d93d37edee98f88eac5c6677d70075.jpg)
...
- 代码实现
  ...
  public class DenseGraph {
      ...
      // 返回图中一个顶点的所有邻边
      // 由于 java 使用引用机制，返回一个 Vector 不会带来额外开销，
...
- 例：
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/e10ecebe96841bf8048ffdf2d18ca161.jpg)

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/89fe259a03d8e1b8f975be7d41098152.jpg)

- 逆邻接表

  为了便于确定顶点的入度或以顶点 vi 为头的弧，可建立一个有向图的逆邻接表，即对每个顶点 vi 建立一个链接以 vi 为头的弧的表。例如，下图所示为有向图 Gl 的逆邻接表：

  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/ec6de105ec5506c013e755262684774c.jpg)
...
- 代码实现
  ...
  public class SparseGraph {
      ...
      public SparseGraph(int n, boolean isDirected) {
          ...
          this.e = 0;    // 初始化没有任何边
          this.isDirected = isDirected;
          // g 初始化为 n 个空的 ArrayList, 表示每一个 g[i] 都为空，即没有任何边
      ...
      // 返回图中一个顶点的所有邻边
      // 由于 java 使用引用机制，返回一个 ArrayList 不会带来额外开销
      public Iterable<Integer> adj(int v) {
          assert v >= 0 && v < n;
          return g[v];
      }
...
- 弧结点
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/63d4f55ea2feacbaf32c80992817c8c4.jpg)
...
- 头结点（顶点结点）
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/da7e3604846b7717abb1c8dbc14eded2.jpg)
...
![image](http://img.cdn.firejq.com/jpg/2018/2/24/6111c9f7108e682435e7e29d829c2a4f.jpg)
...
- 边表结点由以下 6 个域组成：
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/83e89c597e94903807238ee531fbb1ba.jpg)
...
- 顶点结点由以下 2 个域组成：
  ...
  ![image](http://img.cdn.firejq.com/jpg/2018/2/24/3fbe54db92159ce6ccae814bfd500e01.jpg)
...
![image](http://img.cdn.firejq.com/jpg/2018/2/24/06710c2ca10e7ba55a5a731cbb9f0fe6.jpg)
...
![image](http://img.cdn.firejq.com/jpg/2018/4/17/07a8553efaacff3a5a8039626fc9518b.jpg)

基本原理就是取连通分量的过程。

### 3.2. 扫雷

![image](http://img.cdn.firejq.com/jpg/2018/4/17/30a5d79bdcd844d371b05d8db68c3ad7.jpg)
...
![image](http://img.cdn.firejq.com/jpg/2018/4/17/20250fd79d5a0d3d70508aca8375fd7b.jpg)

如上图所示，红色块表示通路，蓝色块表示墙。生成迷宫的过程，只需不断地将蓝色块变为红色块，形成通路。由于使用 BFS 或 DFS 会导致无法生成随机的迷宫，因此应利用一个随机队列，每次从队列中取出一个随机元素进行遍历，进而生成一个随机的迷宫。

### 3.5. 地图填色

![image](http://img.cdn.firejq.com/jpg/2018/4/17/ef14ec95f67ae62b2105018a93709662.jpg)
