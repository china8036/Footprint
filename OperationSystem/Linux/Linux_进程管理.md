...
- `TASK_INTERRUPTIBLE`: 可中断的等待状态。由于进程未获得它所申请的资源而处在等待状态。一旦内核通知资源有效或者有唤醒信号，进程会立即结束等待而进入就绪状态 (TASK_RUNNING)，只要调度器选中该进程即可恢复执行。

- `TASK_UNINTERRUPTIBLE`: 不可中断的等待状态。与 TASK_INTERRUPTIBLE 状态的区别在于：**处于 TASK_UNINTERRUPTIBL 状态的进程不能被异步信号量或中断所唤醒**，只有当它申请的资源有效时通过内核的 wake_up() 调用才能唤醒，而不能由外部信号唤醒。这个状态被应用在内核中某些场景中，比如当进程需要对磁盘进行读写，而此刻正在 DMA 中进行着数据到内存的拷贝，如果这时进程休眠被打断（比如强制退出信号）那么很可能会出现问题，所以这时进程就会处于不可被打断的状态下。
...
- `TASK_ZOMBIE`: 僵死状态。进程虽然已经终止，但由于某种原因，父进程还没有执行 wait() 系统调用，导致虽然进程占有的大部分资源被回收，但其 task_struct 结构（以及少数资源）仍然保存在内存中，成为僵尸进程。处于该状态的进程实际上是系统中的垃圾，必须进行相应处理以释放其占用的资源。
