...
协程不是操作系统的概念，它是属于用户态的东西，和操作系统的进程和线程没有直接关系，你可以 1 个进程上跑 n 个协程，也可以 n 个线程下跑 m 个协程。
...
Stackless Coroutine 指的是同一线程中的 routine 与它的调用者共享一个栈，routine 切换时不保存调用栈以及寄存器等信息，只保存程序计数器，并通过**对象指针**来切换回已保存的 routine。
...
![image](http://otaivnlxc.bkt.clouddn.com/jpg/2018/7/21/c6ce3b863b6bb39a552a0a4aa1b38bcc.jpg)
