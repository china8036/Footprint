...
**网桥不会检查网络层**，所以，IPv4、IPv6、AppleTalk、ATM、IPX、OSI 分组均可穿越网桥。路由器检查逻辑地址（如 IP 地址）作出分组转发的决策。
...
独享式网络的数据传输效率较高，不会出现广播风暴，在安全性方面也不易出现其它节点侦听的现象。
...
- 在 OSI 模型中的工作层次
  - 集线器工作在第一层物理层。
  - 交换机工作在第二层数据链路层，高级交换机可以工作在第三层（网络层）和第四层（传输层）。 
  - 路由器工作在第三层网络层，是互联网的主要骨干。
