- [算法：概述](#)
  ...
  - [3. 算法分析](#3)
    - [3.1. 时间复杂度](#31)
      - [3.1.1. （渐进）时间复杂度](#311)
      - [3.1.2. O(f(n)) 简化规则](#312-ofn)
      - [3.1.3. 时间复杂度分析](#313)
        - [3.1.3.1. 非递归程序](#3131)
        - [3.1.3.2. 递归程序](#3132)
        - [3.1.3.3. 均摊分析 (Amortized Time)](#3133--amortized-time)
      - [3.1.4. 常见数据规模](#314)
    - [3.2. 空间复杂度](#32)
...
若在程序中有递归过程，则**需要根据每次递归中递归调用的次数、每次递归的时间复杂度以及递归的深度进行具体分析**。
...
- 递归中进行多次递归调用
  ...
  如果递归函数中进行了多次递归调用，则递归深度与总递归次数是不同的，因此需要计算递归调用的次数。
