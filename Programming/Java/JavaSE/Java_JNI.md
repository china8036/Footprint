- [Java 本地接口 (Java Native Interface)](#java-本地接口-java-native-interface)
    - [1. 基本概念](#1-基本概念)
        - [1.1. 使用目的](#11-使用目的)
        - [1.2. native 关键字](#12-native-关键字)
        - [1.3. 加载过程](#13-加载过程)
    - [2. JNI 开发](#2-jni-开发)
        - [2.1. 限制](#21-限制)
        - [2.2. 步骤](#22-步骤)
...
如果一个方法描述符内有 native，这个描述符块将有一个指向该方法的实现的指针。这些实现在一些 DLL 文件内，但是它们会被操作系统加载到 Java 程序的地址空间。当一个带有本地方法的类被加载时，其相关的 DLL 并未被加载，因此指向方法实现的指针并不会被设置。当本地方法被调用之前，这些 DLL 才会被加载，这是通过调用 `System.loadLibrary()` 实现的。
