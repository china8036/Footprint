...
NOTE：通配符与正则表达式中的部分元字符相同：`*`, `?`, `[ ]`, `{ }`（没有 `.`，`.` 不是通配符)，但它们是完全不同的概念：
...
- 手机号：`\\b1[35][0-9]\\d{8}|147\\d{8}|1[8][01236789]\\d{8}\\b`.

- 手机号前带 86 或是 +86 的情况（135 号段）: `\\b((\\+86)|(86))?1[35][0-9]\\d{8}|147\\d{8}|1[8][01236789]\\d{8}\\b`.

- 匹配格式：11 位手机号码以及电话号码（3-4 位区号，7-8 位直播号码，1－4 位分机号）；如：12345678901、1234-12345678-1234: `(\\d{11})|^((\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1})|(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1}))$`.

- 18 位身份证：`\\b[1-9]\\d{5}(?:19|20)\\d{2}(?:0[1-9]|[1][012])(?#月)(?:0[1-9]|[12][0-9]|[3][01])(?#日)\\d{3}[\d|X|x]\\b`.

- 15 位身份证：`\\b[1-9]\\d{7}(?:0[1-9]|[1][012])(?#月)(?:0[1-9]|[12][0-9]|[3][01])(?#日)\\d{3}\\b`.

- IPv4 地址：`\\b(?:(?:2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(?:2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\b`.

- 邮箱（不包含教育邮箱等): `\\b(?:[a-zA-Z0-9_-])+@(?:[a-zA-Z0-9_-])+(?:\\.[a-zA-Z0-9_-]{2,3}){1,2}\\b`.

- 包含所有邮箱：`([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)`.
